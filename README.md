...
```bash
# Compare two images
swift run peep before.png after.png

# JSON output
swift run peep before.png after.png --json

# Resize on dimension mismatch (e.g., 1x vs 2x)
swift run peep image-1x.png image-2x.png --resize

# CI threshold — exits non-zero if MAE > 5.0
swift run peep expected.png actual.png --threshold 5.0
```
